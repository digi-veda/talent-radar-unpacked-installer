...
$workDir = Join-Path $env:TEMP ("talent-radar-upgrade-" + [Guid]::NewGuid().ToString("N"))
$zipPath = Join-Path $workDir "talent-radar-unpacked-installer.zip"
$extractDir = Join-Path $workDir "bundle"

if (-not (Test-Path -LiteralPath $targetExtensionDir -PathType Container)) {
  throw "Existing install not found at: $targetExtensionDir. Run self-install-fixed-path.ps1 first."
}
...
Write-Host "Overwriting installed extension files..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Upgrade completed at:" -ForegroundColor Green
...
Write-Host "Click Reload on the Talent Radar extension card." -ForegroundColor Cyan
